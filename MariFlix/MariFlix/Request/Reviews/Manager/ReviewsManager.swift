...
class ReviewsManager {
    ...
    func fetchReviewsMovieWithCompletion(movieID: String, completion: @escaping ([Reviews]?)->Void) {

        //urlCOmponents: URL customizada
        let inicialURL = "https://api.themoviedb.org/3/movie/"
        let finishURL = "/reviews?api_key=19649d836ff40809b43e658f81b875dc"
         let urlStringReviewsMovie = inicialURL + movieID + finishURL
        
        guard let url = URL(string: urlStringReviewsMovie) else {
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            guard let data = data,
                    let response = try? JSONDecoder().decode(ReviewsResponse.self, from: data) else {
                    completion(nil)
                return
            }
